...
contract MemeGuardServiceManagerSetup is Test {
    ...
    function createAssessmentTask(string memory assessmentType, bytes32 targetId, address targetAddress)
        internal
        returns (MemeGuardServiceManager.Task memory)
    {
        IMemeGuardServiceManager serviceManager = IMemeGuardServiceManager(memeGuardDeployment.memeGuardServiceManager);
    ...
    function respondToAssessment(
        ...
        vm.prank(operator.key.addr);
        serviceManager.respondToAssessment(task, taskId, signature, riskScore, isCritical, reportHash);
    }
}
...
contract MemeGuardServiceManagerInitialization is MemeGuardServiceManagerSetup {
    function testInitialization() public view {
        ...
        assertTrue(memeGuardDeployment.memeGuardServiceManager != address(0), "MemeGuardServiceManager not deployed");
...
contract CreateTask is MemeGuardServiceManagerSetup {
    ...
    function testCreateTask() public {
        vm.prank(platform.key.addr);
        MemeGuardServiceManager.Task memory newTask =
            sm.createAssessmentTask("strategy", TEST_STRATEGY_ID, TEST_IMPLEMENTATION);
...
contract RespondToAssessment is MemeGuardServiceManagerSetup {
    ...
    function testRespondToAssessment() public {
        // Create a task
        MemeGuardServiceManager.Task memory task =
            createAssessmentTask("strategy", TEST_STRATEGY_ID, TEST_IMPLEMENTATION);
        ...
        vm.mockCall(
            ...
            abi.encodeWithSelector(
                ECDSAStakeRegistry.isValidSignature.selector, ethSignedMessageHash, bytes("mock_signature")
            ),
        ...
        sm.respondToAssessment(task, taskId, bytes("mock_signature"), riskScore, isCritical, reportHash);
    ...
    function testConsensusReached() public {
        ...
        bytes32 assessmentId = keccak256(abi.encodePacked(task.assessmentType, task.targetId));
        MemeGuardServiceManager.RiskAssessment memory params = sm.consensusAssessments(assessmentId);
    ...
    function testTokenSafetyCheck() public {
        ...
        MemeGuardServiceManager.Task memory task = createAssessmentTask("token", TEST_POOL_ID, TEST_TOKEN_ADDRESS);
        uint32 taskId = sm.latestTaskNum() - 1;
    ...
    function testTransitionReadiness() public {
        ...
        MemeGuardServiceManager.Task memory task = createAssessmentTask("transition", TEST_POOL_ID, TEST_IMPLEMENTATION);
        uint32 taskId = sm.latestTaskNum() - 1;
