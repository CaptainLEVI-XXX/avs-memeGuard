...
library HelloWorldDeploymentLib {
    ...
    function deployAndUpgradeStakeRegistryImpl(
        ...
        bytes memory upgradeCall =
            abi.encodeCall(ECDSAStakeRegistry.initialize, (deployment.memeGuardServiceManager, 0, quorum));
    ...
    function deployAndUpgradeServiceManagerImpl(
        ...
        bytes memory upgradeCall = abi.encodeCall(MemeGuardServiceManager.initialize, (owner, rewardsInitiator));

        UpgradeableProxyLib.upgradeAndCall(memeGuardServiceManager, helloWorldServiceManagerImpl, upgradeCall);
    }
    ...
    function writeDeploymentJson(string memory outputPath, uint256 chainId, DeploymentData memory data) internal {
        address proxyAdmin = address(UpgradeableProxyLib.getProxyAdmin(data.memeGuardServiceManager));
