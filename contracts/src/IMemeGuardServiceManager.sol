...
interface IMemeGuardServiceManager {
    ...
    function operatorAssessments(bytes32 assessmentId, address operator) external view returns (MemeGuardServiceManager.RiskAssessment memory);
    function consensusAssessments(bytes32 assessmentId) external view returns (MemeGuardServiceManager.RiskAssessment memory);
    ...
    ) external returns (MemeGuardServiceManager.Task memory);
